...
from tensorflow.keras.layers import Layer, Dense, LayerNormalization, Dropout, Reshape
...
def construct_model(time_step, embed_dim = 32, num_heads = 8, ff_dim = 512, num_layers = 4, dropout_rate = 0.1):
    ...
    outputs = Dense(48 * 3)(x)
    outputs = Reshape((48, 3))(outputs)
    return Model(inputs, outputs)
@keras.utils.register_keras_serializable()
def pinball_loss(y_actual, y_pred):

    y_actual = tf.expand_dims(y_actual, axis = -1)
    difference = y_actual - y_pred
    losses = tf.where(difference >= 0, QUANTILES * difference, (QUANTILES - 1)* difference)
